...
def update_df_with_rsa(df_rsa, df, unit, net, rsa_values):
    ...
    # assign according to the type and strength, unit and network
    mask = df_rsa.index.map(lambda x: (x[1] in df['type'].values) and (x[2] in df['strength'].values))
    # Assign the rsa_values to the masked rows
    df_rsa.loc[mask, (unit, net)] = rsa_values
...
figures_dir = os.path.join('C:', 'Users', 'gio', 'Data', 'figures', 'silencing_neurips_revision')
...
elif experiment_class == 'imagenet100':
    full_experiment_suffixes = anevo.get_complete_experiment_list(exc_inh_weights=[0.5, 1.0],
                                                                  abs_weights=[])
...
with plt.rc_context(params):
    ...
    if compile_all_units or experiment_class == 'imagenette' or experiment_class == 'imagenet100':
...
#%%
# LPIPS computation between pairs of images
...
def compute_lpips_similarity(df_dict, image_dict, complete_unit_index_list,
                             net_str, layer_str, preprocess_data_to_file=True, preprocessed_data_dir_path=None):
    file_name = f'rsa_{net_str}_{layer_str}_metric_lpips.pkl'
    file_path = join(preprocessed_data_dir_path, file_name)
    # check if the file exists
    if os.path.exists(file_path) and preprocess_data_to_file:
        df_lpips = pd.read_pickle(file_path)
        return df_lpips
    ...
    first_unit = complete_unit_index_list[0]
    df_first_unit = df_dict_copy[first_unit]
    # df_rsa = initialize_df_structure(df_first_unit, , complete_unit_index_list)
    df_lpips = df_first_unit.copy().reset_index().set_index(['type', 'strength'])
    # drop the other columns not in complete_unit_index_list
    df_lpips.reindex(columns=complete_unit_index_list)
    df_lpips = df_lpips.drop(columns=[col for col in df_lpips.columns if col not in complete_unit_index_list])
    ...
    batch_size = 10
    # Load LPIPS model if required
    lpips_model = LPIPS(net='vgg').to(torch.device('cuda:0'))
        # Call function to compute RSA values
    for unit in complete_unit_index_list:
        df = df_dict_copy[unit]
        image_list = image_dict_copy[unit]
        ...
        # Compute LPIPS similarity between all pairs of images
        for img_idx in range(len(image_list)):
            img_tensor = image_list[img_idx].to(torch.device('cuda:0'))
            # print(img_tensor.shape)
            # Compute pairwise LPIPS distances
            n = control_tensor.shape[0]
            # check batch sizes match
            assert n == img_tensor.shape[0]
            pairwise_lpips = torch.zeros((n, n), device='cuda:0')

            # compute lpips for each pair of images of control_tensor and img_tensor
            for i in range(0, n):
                for j in range(0, n):
                    pairwise_lpips[i, j] = lpips_model(control_tensor[i].unsqueeze(0), img_tensor[j].unsqueeze(0)).item()
            # plt.imshow(pairwise_lpips.cpu().numpy())
            # plt.colorbar()
            # plt.show()
            if unit not in df_lpips.columns:
                df_lpips[unit] = np.nan
            if df.loc[img_idx, 'type'] == 'none':
                # average only upper triangular part of the matrix
                df_lpips.loc[(df.loc[img_idx, 'type'], df.loc[img_idx, 'strength']), unit] = \
                    pairwise_lpips[np.triu_indices(n, k=1)].mean().item()

            else:
                # average all the elements of the matrix
                df_lpips.loc[(df.loc[img_idx, 'type'], df.loc[img_idx, 'strength']), unit] = pairwise_lpips.mean().item()
            # df_lpips.loc[img_idx, unit] = pairwise_lpips.mean().item()
            print(f'unit {unit} img_idx {img_idx} mean lpips {pairwise_lpips.mean().item()}')

    # # save the dataframe
    # if preprocess_data_to_file:
    #     df_lpips.to_pickle(file_path)
    return df_lpips

#%%
# compute LPIPS similarity
df_lpips = compute_lpips_similarity(df_dict, image_dict, complete_unit_index_list,
                                    net_str, layer_str, preprocess_data_to_file=True, preprocessed_data_dir_path=preprocessed_data_dir_path)
#%%

formatted_lpips_df = df_lpips.stack().reset_index(name='mean_lpips')
# rename column level_3 to unit
formatted_lpips_df.rename(columns={'level_3': 'unit'}, inplace=True)
# convert to 1 - mean_lpips to get similarity, call new column similarity
...
palette = ['tab:orange', 'tab:green', 'tab:gray']
...
# location outside the plot, no box
ax.legend(handles[0:3], labels[0:3], bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.)
# ylabel similairty(1 - LPIPS)
plt.ylabel('Similarity (1 - LPIPS)')
# title of the plot Perceptual similarity control vs ablation
plt.title('Perceptual similarity \ncontrol vs ablation')
plt.tight_layout()
# save pdf and png high resolution
fig.savefig(join(figures_dir, f'{net_str}_{layer_str}_lpips_similarity.png'), dpi=300)
fig.savefig(join(figures_dir, f'{net_str}_{layer_str}_lpips_similarity.pdf'), dpi=300)
...
with plt.rc_context(params):
    ...
    for itype, type in enumerate(['all', 'inh', 'exc']):
